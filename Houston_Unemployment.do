...
ac unemp
pac unemp
varsoc unemp, maxlag(20)
dfuller unemp, lag(15) trend
dfuller unemp, lag(15)
dfuller unemp, lag(15) nocons

*Perron’s approach - To account for structural variation in unemployment
drop Uemp_st
g dumy=1 if time<200
replace dumy =0 if time>=200
g dum=0 if time<100
replace dum =1 if time>=100
regress unemp dum dumy time
predict ures , resid
twoway (line ures time), xtitle(time)

g Uemp_st=4.23+ures

twoway (line Uemp_st time), xtitle(time)


*DF test of detrended Real Price
ac ures
pac ures
varsoc ures, maxlag(20)
dfuller ures, lag(12) trend
dfuller ures, lag(12) 
dfuller ures, lag(12) nocons  /*Not able to reject unit root*/
...
**********************************VARX*************************************************
g upd=Uemp_st[_n-1]*PD
g utd=Uemp_st[_n-1]*TD
g opd=dOil[_n-1]*PD
g otd=dOil[_n-1]*TD
g gpd=dGas[_n-1]*PD
g gtd=dGas[_n-1]*TD
...
varsoc Uemp_st dOil Gas_st dCot /* this gives us 1 lag */


var Uemp_st dOil Gas_st dCot, lags(1) exog(PD TD upd utd opd otd gpd gtd cpd ctd)
varstable
drop TD PD upd utd opd otd gpd gtd cpd ctd
